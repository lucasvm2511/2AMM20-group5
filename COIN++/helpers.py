...
def dataset_name_to_dims(dataset_name):
    ...
    if dataset_name == "era5":
        dim_in = 3
        dim_out = 1
    if dataset_name == "EuroSatDataset":
        dim_in = 2
        dim_out = 3
    if dataset_name == "EuroSatallDataset":
        dim_in = 2
        dim_out = 13
    if dataset_name == "LargeDataset":
        dim_in = 2
        dim_out = 13 # change to 3 for RGB
...
def get_datasets_and_converter(args, force_no_random_crop=False):
    ...
    if "EuroSatDataset" in (args.train_dataset, args.test_dataset):
        converter = conversion.Converter("image")

        if args.train_dataset == "EuroSatDataset":
            transforms = [torchvision.transforms.ToTensor()]
            if use_patching and not force_no_random_crop:
                transforms.append(random_crop)

            train_dataset = image.EuroSatDataset(
                df_path=os.getcwd()+'/data/EuroSat/EuroSAT/train.csv',
                transform=torchvision.transforms.Compose(transforms),
            )

        if args.test_dataset == "EuroSatDataset":
            test_dataset = image.EuroSatDataset(
                df_path=os.getcwd()+'/data/EuroSat/EuroSAT/train.csv',
                transform=torchvision.transforms.ToTensor(),
            )

    if "EuroSatallDataset" in (args.train_dataset, args.test_dataset):
        converter = conversion.Converter("image")

        if args.train_dataset == "EuroSatallDataset":
            transforms = [torchvision.transforms.ToTensor()]
            if use_patching and not force_no_random_crop:
                transforms.append(random_crop)

            train_dataset = image.EuroSatallDataset(
                df_path=os.getcwd()+'/data/EuroSat/EuroSATallBands/train.csv',
                transform=torchvision.transforms.Compose(transforms),
            )

        if args.test_dataset == "EuroSatallDataset":
            test_dataset = image.EuroSatallDataset(
                df_path=os.getcwd()+'/data/EuroSat/EuroSATallBands/test.csv',
                transform=torchvision.transforms.ToTensor(),
            )
    if "LargeDataset" in (args.train_dataset, args.test_dataset):
        converter = conversion.Converter("image")

        if args.train_dataset == "LargeDataset":
            transforms = [torchvision.transforms.ToTensor()]
            if use_patching and not force_no_random_crop:
                transforms.append(random_crop)

            train_dataset = image.LargeDataset(
                df_path=os.getcwd()+'/data/np_downscaled_and_cropped/train.csv',
                transform=torchvision.transforms.Compose(transforms),
            )

        if args.test_dataset == "LargeDataset":
            test_dataset = image.LargeDataset(
                df_path=os.getcwd()+'/data/np_downscaled_and_cropped/test.csv',
                transform=torchvision.transforms.ToTensor(),
            )
