...
if __name__ == "__main__":
    wandb.login(key='c993520c83451ca7800ca650def20cecb982205e')
    import argparse
    import os
    import wandb
    import wandb_utils
    ...
    for idx in args.data_indices:
        ...
        if converter.data_type == "image":
            ...
            try:
                ...
                tiff.imwrite(os.path.join(args.save_dir, f"original_{idx}.tiff"), data.cpu().numpy() )
                tiff.imwrite(os.path.join(args.save_dir, f"reconstruction_{idx}.tiff"), data_recon.cpu().numpy() )
