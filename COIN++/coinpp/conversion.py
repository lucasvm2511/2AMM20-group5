...
def features2data(features, batched=False):
    ...
    if batched:
        # Move channels dimension to first non-batch axis
        data = torch.moveaxis(features, -1, 1)
    else:
        data = torch.moveaxis(features, -1, 0)
    
    # Check the number of channels
    channels = data.shape[1] if batched else data.shape[0]
    
    if channels > 3: # Multichannel image
        indices = [3, 2, 1] #Get RGB channels for wandb
        data = data[:, indices, ...] if batched else data[indices, ...]
